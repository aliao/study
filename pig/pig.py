...
def take_turn(plan, dice=make_fair_die(), who='Someone', comments=False):
    ...
    while not is_turn_over:
        ...
        outcome = dice()
        score_for_turn, turn_total, is_turn_over = action(turn_total, outcome)
        if comments:
            commentate(action, outcome, score_for_turn, turn_total, is_turn_over, who)
...
def draw_number(n, dot='*'):
    ...
    """
    if n == 1:
        return draw_die(True, False, False, False, '*')
    elif n == 2:
        return draw_die(False, True, False, False, '*')
    elif n == 3:
        return draw_die(True, True, False, False, '*')
    elif n == 4:
        return draw_die(False, True, True, False, '*')
    elif n == 5:
        return draw_die(True, True, True, False, '*')
    elif n == 6:
        return draw_die(False, True, True, True, '*')
    #return ''
...
def run_strategy_experiments():
    ...
    #best_value = eval_strategy_range(make_roll_until_strategy, 15, 25)
    #print('The best value is:', best_value)
    #best_value = eval_strategy_range(make_die_specific_strategy, 5, 15)
    #print('The best value is:', best_value)
    #best_value = eval_strategy_range(make_pride_strategy, 0, 10)
    #print('The best value is:', best_value)
    #best_value = eval_strategy_range(make_final_strategy, 0, 20)
    #print('The best value is:', best_value)
...
def make_pride_strategy(margin, turn_goal=20):
    ...
    def strategy(player_score, opponent_score):
        def plan(turn):
            if turn >= turn_goal and turn + player_score - opponent_score >= margin:
                return hold
            else:
                return roll
        return plan
