    ...
    def advance(self):
        ...
        for row in range(1, self.H - 1):
            for col in range(1, self.W - 1):
                ...
                for i in range(-1, 2):
                    if self.B0[row - 1][col + i] == '*':
                        neighbors_occupied += 1
                    if self.B0[row + 1][col + i] == '*':
                        neighbors_occupied += 1
                if self.B0[row][col - 1] == '*':
                    neighbors_occupied += 1
                if self.B0[row][col + 1] == '*':
                    neighbors_occupied += 1
                if self.B0[row][col] == '*':
                    if neighbors_occupied in (2, 3):
                        self.B1[row][col] = '*'
                    else:
                        self.B1[row][col] = '.'
                elif self.B0[row][col] == '.':
...
def dfilter_rlist(pred, r):
    ...
    if r == empty_rlist:
        ...
        if pred(first(r)):
            set_rest(r, dfilter_rlist(pred, rest(r)))
        else:
            r = dfilter_rlist(pred, rest(r))
        return r
...
def dshuffle_rlist(r):
    ...
    [0, 4, 1, 5, 2, 6, 3]
    """
    result_list = shuffle(rlist_to_list(r))
    for i in range(len(result_list)):
        target = r
        for j in range(i):
            target = rest(target)
        set_first(target, result_list[i])
    return r
