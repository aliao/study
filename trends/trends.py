...
def extract_words(text):
    ...
    for i in range(len(text_list)):
        if text_list[i] not in string.ascii_letters:
            text_list[i] = ' '
    text = ''.join(text_list)
    return text.split()
...
def analyze_tweet_sentiment(tweet):
    ...
    """
    average = None
    number_of_sentiment_word, sentiment_total = 0, 0
    for word in tweet_words(tweet):
        sentiment = get_word_sentiment(word)
        if sentiment:
            sentiment_total += sentiment
            number_of_sentiment_word += 1
    if number_of_sentiment_word:
        average = sentiment_total / number_of_sentiment_word
    return average
...
def find_centroid(polygon):
    ...
    if area != 0:
        for i in range(len(x) - 1):
            c_x += (x[i] + x[i + 1]) * (x[i] * y[i + 1] - x[i + 1] * y[i]) / (6 * area)
            c_y += (y[i] + y[i + 1]) * (x[i] * y[i + 1] - x[i + 1] * y[i]) / (6 * area)
...
def calculate_average_sentiments(tweets_by_state):
    ...
    for n, tweets in tweets_by_state.items():
        state_sentiment = 0
        for tweet in tweets:
            sentiment = analyze_tweet_sentiment(tweet)
            if sentiment:
                state_sentiment += sentiment
        if not state_sentiment:
            averaged_state_sentiments[n] = state_sentiment
